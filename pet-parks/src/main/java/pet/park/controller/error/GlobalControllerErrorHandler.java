...
public class GlobalControllerErrorHandler {
	...
	} // (VIDEO 14-5 Delete Contributor) - unsupported exception to delete all
		// contributors

	@ExceptionHandler(Exception.class)
	@ResponseStatus(code = HttpStatus.INTERNAL_SERVER_ERROR)
	public ExceptionMessage handldeException(Exception ex, WebRequest webRequest) {
	...
	} // (14-3 Global Error Handler) - creating a message to decipher error code
	...
	private ExceptionMessage buildExceptionMessage(Exception ex, HttpStatus status, WebRequest webRequest,
		...
		excMsg.setUri(uri);
		return excMsg;
	} // (14-3 Global Error Handler) - creating a method to decipher error code with
		// reasons noted above in exe.Msg
